  ...
  <div class="app-container" style="padding: 8px;">
      ...
      <el-form ref="form" :model="form" size="small" label-width="80px">
        ...
        <el-form-item v-if="crud.status.add" label="上传">
          <el-upload
            ...
            :action="objUploadApi + '?name=' + form.fileName"
          >
    ...
    >
      ...
      <el-table-column label="地址">
        <template slot-scope="scope">
          {{ scope.row.endpoint + '/' + scope.row.objBucket + '/' + scope.row.objName }}
        </template>
      </el-table-column>
      <el-table-column label="预览图" width="100">
        <template slot-scope="{row}">
          <el-image
            v-if="row.fileType === 'image' && row.suffix !== 'psd'"
            :src=" row.endpoint + '/' + row.objBucket + '/' + row.objName"
            :preview-src-list="[row.endpoint + '/' + row.objBucket + '/' + row.objName]"
            fit="contain"
            class="el-avatar"
          >
            <div slot="error">
      ...
      <el-table-column prop="fileType" label="类别" width="100" />
      <el-table-column prop="fileSizeDesc" label="大小" width="100" />
      <el-table-column prop="createBy" label="操作人" width="100" fixed="right" />
      <el-table-column prop="createTime" label="创建日期" width="150" fixed="right" />
      <el-table-column label="操作" width="100" fixed="right">
        <template slot-scope="{row}">
          <el-button type="text" @click="downloadFile(row.id)">下载</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
...
import crudFile from '@/api/tools/minioStorage'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '文件', url: 'api/minioStorage', crudMethod: { ...crudFile }})
  },
  ...
  methods: {
    ...
    downloadFile(id) {
      crudFile.downloadFile(id).then(shareUrl => {
        // minio生成文件共享链接
